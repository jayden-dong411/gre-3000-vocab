...
底部四个入口：今日、学习、复习、词库。今日页是进度看板，不带复习侧栏。学习页从上到下是单词、释义、不认识 / 模糊 / 认识。释义默认收起，点卡片或按空格后向下展开；宽屏时右侧是到期复习，单词在上、四个选项纵向排列，复习区约占一半宽度。词库按未学、复习中、已掌握筛选，点开单词才显示释义，进入 7 天档记为已掌握。

- 新词：空格或回车展开释义，`1` 不认识，`2` 模糊，`3` 认识
...
这是未公证应用的拦截，不是压缩包坏了。安装包只含 Electron 运行时和打好的页面（词库约 2MB）。仓库里的 `release/` 不会进 git，安装包只走 [GitHub Releases](https://github.com/jayden-dong411/gre-3000-vocab/releases/latest)。
